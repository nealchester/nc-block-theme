...
function nc_gallery_block() {
        ...
        acf_register_block_type(array(
            'name'              => 'nc_gallery',
            'title'             => __('NC Gallery', 'nc-block-theme'),
            'description'       => __('A gallery of images', 'nc-block-theme'),
	...
	<div id="<?php echo $id; ?>" class="ncgallery_container<?php echo esc_attr($className); ?>" <?php echo nc_block_attr();?>>
			...
			?>
				...
				<figure class="ncgallery_item ncgallery_item-<?php echo $i++;?>">
					...
					<?php elseif($link2img == 'image_page'):?><a class="ncgallery_link" aria-label="<?php _e('View larger','nc-block-theme');?>" href="<?php echo get_attachment_link($image['ID']); ?>">
					<?php elseif($link2img == 'image_lightbox'):?><a data-title="<?php echo $image['title']; ?>" aria-label="<?php _e('View larger','nc-block-theme');?>" class="ncgallery_link <?php echo'ncgimg_'.$id; ?>" href="<?php echo wp_get_attachment_image_url($image['ID'], 'large'); ?>"><?php endif;?>
			...
			<?php else:?>
				...
				<div class="nocontent">
					<p><?php _e('Add some images to start. Use the sidebar settings to begin.','nc-block-theme');?></p>
				</div>
